...
from train.train import train, test,test_after_aggregation
...
# parser.add_argument('-bp', '--base-path', default="/root/autodl-tmp")
...
parser.add_argument('-j', '--workers', default=20, type=int, metavar='N',
                    help='number of data loading workers (default: 8)')#8
...
def main(args=args, configs=configs):
    ...
    classifiers_deputy = []
    optimizers_deputy = []
    classifier_optimizers_deputy = []
    optimizer_schedulers_deputy = []
    classifier_optimizer_schedulers_deputy = []
    ...
    if configs["DataConfig"]["dataset"] == "DigitFive":
        ...
        models_deputy.append(CNN(args.data_parallel).cuda())
        classifiers_deputy.append(Classifier(args.data_parallel).cuda())
        ...
        #目标域的副模型和分类器，其实没用到，用来占个位。
        models_deputy.append(
            DomainNet(configs["ModelConfig"]["backbone"], args.bn_momentum, configs["ModelConfig"]["pretrained"],
                      args.data_parallel).cuda())
        classifiers_deputy.append(DomainNetClassifier(configs["ModelConfig"]["backbone"], 345, args.data_parallel).cuda())
        ...
        for domain in domains:
            ...
            # 源域创建副模型 -- 目前比个性化模型少一项。。
            models_deputy.append(DomainNet(configs["ModelConfig"]["backbone"], args.bn_momentum,
                                    pretrained=configs["ModelConfig"]["pretrained"],
                                    data_parallel=args.data_parallel).cuda())
            classifiers_deputy.append(DomainNetClassifier(configs["ModelConfig"]["backbone"], 345, args.data_parallel).cuda())
    ...
    for model in models_deputy:
        optimizers_deputy.append(
            torch.optim.SGD(model.parameters(), momentum=args.momentum,
                            lr=configs["TrainingConfig"]["learning_rate_begin"], weight_decay=args.wd))
    for classifier in classifiers_deputy:
        classifier_optimizers_deputy.append(
            torch.optim.SGD(classifier.parameters(), momentum=args.momentum,
                            lr=configs["TrainingConfig"]["learning_rate_begin"], weight_decay=args.wd))
    ...
    for classifier_optimizer in classifier_optimizers:
        classifier_optimizer_schedulers.append(
            CosineAnnealingLR(classifier_optimizer, configs["TrainingConfig"]["total_epochs"],
                              eta_min=configs["TrainingConfig"]["learning_rate_end"]))
    for optimizer in optimizers_deputy:
        optimizer_schedulers_deputy.append(
            CosineAnnealingLR(optimizer, configs["TrainingConfig"]["total_epochs"],
                              eta_min=configs["TrainingConfig"]["learning_rate_end"]))
    for classifier_optimizer in classifier_optimizers_deputy:
        classifier_optimizer_schedulers_deputy.append(
            CosineAnnealingLR(classifier_optimizer, configs["TrainingConfig"]["total_epochs"],
                              eta_min=configs["TrainingConfig"]["learning_rate_end"]))
    ...
    # adjust training strategy with communication round训练策略和通信轮次
    #   batch_size: 50
    ...
    for epoch in range(args.start_epoch, total_epochs):#80
        ...
        domain_weight = train(train_dloaders, models_deputy, models, classifiers_deputy, classifiers, optimizers_deputy, optimizers,
                              classifier_optimizers_deputy, classifier_optimizers,
                              optimizer_schedulers_deputy,optimizer_schedulers,
                              classifier_optimizer_schedulers_deputy, classifier_optimizer_schedulers,
                              DET_stages, epoch, writer, num_classes=num_classes,
        ...
        # 应该测试个性化模型的性能？但是源域还是用的副模型
        # 1、直接复制源域的副模型到个性化上？如何复制？是否还需要进行个性化操作？
        # 复制参考聚合过程赋值--model.load_state_dict(dict_list[0])?
        # 或者直接复制副模型的list[0]?--应该可行
        # 2、分开测试--单独测试副模型的第一项和个性化模型的后面几项

        test_after_aggregation(args.target_domain, args.source_domains, test_dloaders, models_deputy ,models,classifiers_deputy, classifiers, epoch,
             writer, num_classes=num_classes, top_5_accuracy=(num_classes > 10))
        test(args.target_domain, args.source_domains, test_dloaders, models_deputy,models,classifiers_deputy, classifiers, epoch,
             writer, num_classes=num_classes, top_5_accuracy=(num_classes > 10))

        # 应该测试个性化模型的性能

        # 即先调用 optimizer.step() 再调用 scheduler.step()
        # for scheduler in optimizer_schedulers:
        #     scheduler.step()
        # for scheduler in classifier_optimizer_schedulers:
        #     scheduler.step()
        # for scheduler in optimizer_schedulers_deputy:
        #     scheduler.step()
        # for scheduler in classifier_optimizer_schedulers_deputy:
        #     scheduler.step()
