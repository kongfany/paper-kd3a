...
def train(train_dloader_list,model_deputy_list, model_list, classifier_deputy_list,classifier_list,optimizer_deputy_list, optimizer_list,
    ...
    for f in range(model_aggregation_frequency):# 5
        ...
        for train_dloader, model, classifier, optimizer, classifier_optimizer,optimizer_scheduler,classifier_optimizer_scheduler,model_deputy,classifier_deputy,\
            ...
            #  DET状态判断
            train_acc, train_f1 = test_source(source_domain, train_dloader, model, classifier, num_classes)
            train_acc_deputy, train_f1_deputy = test_source(source_domain, train_dloader, model_deputy,
                                                            classifier_deputy, num_classes)
...
def test(target_domain, source_domains, test_dloader_list,model_deputy_list, model_list,classifier_deputy_list, classifier_list, epoch, writer, num_classes=126,
         top_5_accuracy=True):
    # 分开测试--单独测试副模型的第一项和个性化模型的后面几项
    ...
    for _, (image_t, label_t) in enumerate(test_dloader_t):
        ...
        with torch.no_grad():
            output_t = classifier_deputy_list[0](model_deputy_list[0](image_t))
    ...
    for s_i, domain_s in enumerate(source_domains):
        ...
        if top_5_accuracy:
            ...
            writer.add_scalar(tag="Test/source_domain_{}_accuracy_top5".format(domain_s), scalar_value=top_5_accuracy_s,
                              global_step=epoch + 1)
# 聚合后模型的性能，副模型的性能。
def test_after_aggregation(target_domain, source_domains, test_dloader_list,model_deputy_list, model_list,classifier_deputy_list, classifier_list, epoch, writer, num_classes=126,
         top_5_accuracy=True):
    source_domain_losses = [AverageMeter() for i in source_domains]
    ...
    for s_i, domain_s in enumerate(source_domains):
        ...
        for _, (image_s, label_s) in enumerate(test_dloader_s):
            ...
            label_s = label_s.long().cuda()
            with torch.no_grad():
                output_s = classifier_deputy_list[s_i + 1](model_deputy_list[s_i + 1](image_s))
            label_onehot_s = torch.zeros(label_s.size(0), num_classes).cuda().scatter_(1, label_s.view(-1, 1), 1)
            task_loss_s = task_criterion(output_s, label_s)
            source_domain_losses[s_i].update(float(task_loss_s.item()), image_s.size(0))
            tmp_score.append(torch.softmax(output_s, dim=1))
        ...
        if top_5_accuracy:
            ...
            print("deputy model: Source Domain {} Accuracy {:.3f}".format(domain_s, top_1_accuracy_s))



# train_acc,train_f1 = test_source(source_domain, train_dloader, model, classifier,345)
def test_source(source_domain, test_dloader, classifier, model, num_classes=345):

    source_domain_losses = AverageMeter()
    task_criterion = nn.CrossEntropyLoss().cuda()
    model.eval()
    classifier.eval()
    tmp_score = []
    ...
    for _, (image_s, label_s) in enumerate(test_dloader):
        # 循环遍历测试集数据加载器中的每个batch，每个batch中包含一批测试图片和对应的标签。
        image_s = image_s.cuda()
        label_s = label_s.long().cuda()
        # (Pdb) p image_s.shape
        ...
        with torch.no_grad():
            ...
            # torch.Size([50, 345])
        label_onehot_s = torch.zeros(label_s.size(0), num_classes).cuda().scatter_(1, label_s.view(-1, 1), 1)
        # (Pdb) p label_onehot_s.shape
        # torch.Size([50, 345])
        task_loss_s = task_criterion(output_s, label_s)
        # (Pdb) p task_loss_s.shape
        # torch.Size([])
        # (Pdb) p task_loss_s
        # tensor(5.9552, device='cuda:0')

        # (Pdb) p task_loss_s
        # tensor(2.3007, device='cuda:0')
        # (Pdb) p output_s.shape
        # torch.Size([128, 10])
        # (Pdb) p label_onehot_s.shape
        # torch.Size([128, 345])
        # (Pdb) p torch.sum(y_true == y_pred)
        # tensor(2479, device='cuda:0')
        # (Pdb) p top_1_accuracy_s
        # 0.09916


        # (Pdb) p output_s[0]
        # tensor([-0.0136,  0.0032,  0.0198, -0.0060,  0.0178,  0.0104,  0.0139, -0.0171,
        #         -0.0151,  0.0098], device='cuda:0'
        # (Pdb) p label_onehot_s[0]
        ...
        source_domain_losses.update(float(task_loss_s.item()), image_s.size(0))
        # (Pdb) p float(task_loss_s.item())
        # 5.9552202224731445
        # (Pdb) p image_s.size(0)
        # 50
        # (Pdb) p source_domain_losses
        # <lib.utils.avgmeter.AverageMeter object at 0x7f469563c250>
        tmp_score.append(torch.softmax(output_s, dim=1))
        # turn label into one-hot code
        tmp_label.append(label_onehot_s)
    tmp_score = torch.cat(tmp_score, dim=0).detach()
    ...
    _, y_true = torch.topk(tmp_label, k=1, dim=1)
    _, y_pred = torch.topk(tmp_score, k=1, dim=1)
    #     # (Pdb) p tmp_score.shape
    #     # torch.Size([36023, 345])
    #     # (Pdb) p tmp_label.shape
    #     # torch.Size([36023, 345])
    #     # (Pdb) p y_true.shape
    #     # torch.Size([36023, 1])

    # top_1_accuracy_s = float(torch.sum(y_true == y_pred[:, :1]).item()) / y_true.size(0)
    top_1_accuracy_s = float(torch.sum(y_true == y_pred).item()) / y_true.size(0)
    # (Pdb) p torch.sum(y_true == y_pred[:, :1]).item()
    # 79
    # (Pdb) p  torch.sum(y_true == y_pred).item()
    # 79
    # (Pdb) p y_true.size(0)
    # 36023
    # (Pdb) p top_1_accuracy_s
    # 0.0021930433334258668
    # f1_macro
    y_true_fl = y_true.view(-1).cpu().numpy()
    # y_pred_fl = y_pred[:, :1].view(-1).cpu().numpy()
    y_pred_fl = y_pred.view(-1).cpu().numpy()
    f1_macro = 100 * f1_score(y_true_fl, y_pred_fl, average='macro')
    # (Pdb) p y_true_fl
    # array([143, 215, 214, ...,  88, 310, 313])
    # (Pdb) p y_pred_fl
    # array([ 23,   0, 304, ...,  15,  15,  96])


    # TTest,before train:::Source Domain infograph Accuracy Top1 :0.002 fl:0.035
    return top_1_accuracy_s, f1_macro  # acc and fl