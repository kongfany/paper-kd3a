...
def train(train_dloader_list,model_deputy_list, model_list, classifier_deputy_list,classifier_list,optimizer_deputy_list, optimizer_list,
    ...
    for f in range(model_aggregation_frequency):
        print("f:{}/{}".format(f,model_aggregation_frequency))
        # det 判断
        train_acc, train_f1 = test_target( train_dloader_list[0], model_list[0], classifier_list[0], num_classes)
        train_acc_deputy, train_f1_deputy = test_target( train_dloader_list, model_deputy_list[0],
                                                        classifier_deputy_list[0], num_classes)

        alpha1 = 0.7
        alpha2 = 0.9
        if (train_f1_deputy < alpha1 * train_f1) or det_tar == 0:
            det_tar = 1
            # 2/3 增加鲁棒性， 22 33 22 111          epoch 下 第一次判断的时候增加一个周期性的变化。
        elif (train_f1_deputy >= alpha1 * train_f1 and det_tar == 1) or (
                det_tar >= 2 and train_f1_deputy < alpha2 * train_f1):
            det_tar = 2
        elif train_f1_deputy >= alpha2 * train_f1 and det_tar >= 2:
            det_tar = 3
        else:
            det_tar = 4
        print("Before train--Target Domain Accuracy:{:.3f}, Deputy model:{:.3f}, DET_stage:{}".format(train_f1,train_f1_deputy,det_tar))
        model_list[0].train()
        classifier_list[0].train()

        model_deputy_list[0].train()
        classifier_deputy_list[0].train()
        ...
        for i, (image_t, label_t) in enumerate(train_dloader_list[0]):
            ...
            # 副模型的训练
            feature_t_deputy = model_deputy_list[0](mixed_image)
            output_deputy_t_1 = classifier_deputy_list[0](feature_t_deputy)
            output_deputy_t = torch.log_softmax(output_deputy_t_1, dim=1)
            # 个性化模型的训练
            feature_t = model_list[0](mixed_image)
            output_t_1 = classifier_list[0](feature_t)
            output_t =torch.log_softmax(output_t_1, dim=1)

            # task_loss_t = torch.mean(consensus_weight * torch.sum(-1 * mixed_consensus * output_deputy_t, dim=1))
            # task_loss_t.backward()
            if det_stage == 1:
                # personalized is teacher
                task_loss_t = torch.mean(consensus_weight * torch.sum(-1 * mixed_consensus * output_t, dim=1))
                loss_tar = task_loss_t

                task_loss_t_deputy= torch.mean(consensus_weight * torch.sum(-1 * mixed_consensus * output_deputy_t, dim=1))
                loss_tar_deputy_kl = F.kl_div(F.log_softmax(output_deputy_t_1, dim=1),
                                          F.softmax(output_t_1.clone().detach(), dim=1), reduction='batchmean')

                loss_tar_deputy = task_loss_t_deputy + loss_tar_deputy_kl

            elif det_stage == 2:
                # mutual learning DET_stage = 2
                task_loss_t = torch.mean(consensus_weight * torch.sum(-1 * mixed_consensus * output_t, dim=1))
                loss_tar_kl = F.kl_div(F.log_softmax(output_t_1, dim=1),
                                              F.softmax(output_deputy_t_1.clone().detach(), dim=1), reduction='batchmean')
                loss_tar = task_loss_t + loss_tar_kl

                task_loss_t_deputy = torch.mean(
                    consensus_weight * torch.sum(-1 * mixed_consensus * output_deputy_t, dim=1))
                loss_tar_deputy_kl = F.kl_div(F.log_softmax(output_deputy_t_1, dim=1),
                                              F.softmax(output_t_1.clone().detach(), dim=1), reduction='batchmean')
                ...
                task_loss_t_deputy = torch.mean(
                    consensus_weight * torch.sum(-1 * mixed_consensus * output_deputy_t, dim=1))
                loss_tar_deputy = task_loss_t_deputy
    ...
    federated_average(model_deputy_list, domain_weight, batchnorm_mmd=batchnorm_mmd)
...
def test_target( test_dloader_list, model, classifier, num_classes=126):
    ...
    y_true_fl = y_true.view(-1).cpu().numpy()

    y_pred_fl = y_pred.view(-1).cpu().numpy()
    f1_macro = 100 * f1_score(y_true_fl, y_pred_fl, average='macro')



    # TTest,before train:::Source Domain infograph Accuracy Top1 :0.002 fl:0.035
    return top_1_accuracy_t, f1_macro  # acc and fl
